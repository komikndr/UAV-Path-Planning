...
\chapter{Literature Review}
    ...
    \section{Ground risk model}
        ...
        \begin{equation}\label{eq:general_probability}
            P_{fatal}(x) = P_{failure}(x) \cdot P_{impact}(x) \cdot P_{recovery} \cdot P_{stress} \cdot P_{exposure} \cdot P_{incident}(x) \cdot P_{harm}(x)
        \end{equation}
        ...
        \subsection{Impact location model}
            ...
            on the possible maximum UAV descent trajectory distance. The impact location model can be formulated using
            two different models, hypothetical and empirical model.

            Generally speaking, the hypothetical model used in research is often tries to model a relationship between
            the size of the UAV to the sizes of its impacted area. Some model such as from \cite{primatesta_ground_2020}
            and \cite{burke_system-level_2011} uses the aerodynamic model to simulate the behavior of the impact
            location. This model can be further separated into a ballistic descent, gliding descent, and planform model.
            The ballistic descent calculated the trajectory based on the ballistic profile. The gliding model such as in
            assumes the UAV will continue descending based on their glide profile. And the planform model uses the
            aircraft wings and its layout aerodynamic properties to calculate the descent trajectory. 
            
            According to \cite{primatesta_risk-based_2020} it is very expensive to calculate full aerodynamic properties
            per cell in large number of cells in a grid, therefore it is not surpriseing many research use only gliding
            and ballistic descent to calculate the impact model. 
