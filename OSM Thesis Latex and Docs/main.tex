...
\chapter{Literature Review}
    ...
    \section{Pathfinding Algorithms}
        ...
        inappropriate heuristic can degrade the efficiency \cite{knuth_art_1997}.
        ...
        convergence rates. But we are not going to dive deeper into these extension since RRT itself is for real time on
        the fly path finder, where our paper is for calculating pre-flight ground risk model.
        ...
        pathfinding applications. \cite{lavalle_planning_2006}.
        ...
        problems such as the travelling salesman problem \cite{dorigo_ant_2004}. ACO simulates the behavior of ants
        ...
        In layman's terms, the ACO algorithm can be likened to a group of ants exploring different paths to find the
        shortest route to food. As ants travel, they leave a trail (pheromone) that other ants can follow. Over time,
        the paths with stronger pheromone trails attract more ants, reinforcing good paths and eventually leading to the
        discovery of the optimal route.
        ...
        resource-constrained settings. Additionally, the performance of ACO is sensitive to parameter settings,
        necessitating careful tuning to achieve optimal results. There is also a risk of converging to a suboptimal
        solution if the algorithm is not properly managed, highlighting the importance of adequate oversight and
        control.
        
        In conclusion, the ACO algorithm is a robust and flexible optimization technique that mimics the natural
        behavior of ants to solve complex problems. Its ability to adapt and find near-optimal solutions makes it a
        valuable tool in various applications \cite{dorigo_ant_2004}.
        ...
        The generative model's objective is to learn the distribution of optimal paths. For instance, in the case of
        GANs, the generator \( G \) aims to produce paths that are indistinguishable from real paths, while the
        discriminator \( D \) tries to distinguish between real and generated paths. The loss function for GANs is given
        by:
        \begin{equation}
        ...
        where:
        \begin{itemize}
          \item \( x \) represents real paths.
          \item \( z \) is a random noise vector.
          \item \( p_{\text{data}}(x) \) is the distribution of real paths.
          \item \( p_z(z) \) is the distribution of the noise vector.
        \end{itemize}
        
        In layman's terms, Generative Pathfinding AI can be thought of as an intelligent system that learns from
        examples of good paths and then creates new paths that are similar to the ones it learned. It's like a student
        learning to solve maze puzzles by looking at many examples and then being able to solve new mazes it has never
        seen before.
