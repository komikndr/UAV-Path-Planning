...
\chapter{Literature Review}
    ...
    \section{Ground risk model}
        ...
        graph, where we start at the root node and traverse based on yes/no questions. These questions can range from
        ...
        This brings us to the most widely used method, the use of ground probabilistic risk assessment methods, which
        ...
        figure \ref{fig:prime_example} is derived from the prime example of this research conducted by
        \cite{primatesta_ground_2020}. 
        ...
        A ground probabilistic risk assesment model, shortened to ground risk model, of an UAV typically consists of
        several subcomponents that are used to assess the various factors that can impact the safety and effectiveness
        of UAV operations. These components in ground risk model are, failure model, impact location model, recovery
        model, stress model, exposure model, incident stress model, and harm model, where it will be explain in further
        below.
        
        However, this approach is not new, since many earlier researches already follow a guideline to model these
        components. For example \cite{breunig_modeling_2018} provide general guideline for the model, while
        \cite{washington_review_2017} provide a listing of different kind of research using more or less the same
        paradign to  describe the model. Where \cite{washington_review_2017} in itself use the method of compilation is
        based on \cite{pat-cornell_uncertainties_1996}, which studied the level of analytical detail when modeling the
        risk. We derived our study based on the \cite{primatesta_ground_2020} is simply because it shows the most
        comprehensive way to calculate the ground risk model. Which can be summerized into equation
        ...
        ground risk model, and often time use different methodology to analysize the same component. For example,
        \cite{primatesta_ground_2020} only use event, impact location, and fatality model, while
        \cite{kim_risk-based_2022} use event, impact, exposure, incident stress, and harm model. Despite in a glance
        that there are difference in modeling the ground risk, sometimes the researcher combine multiple subcomponent as
        one. Again for example in \cite{primatesta_ground_2020} fatality model component is in fact a combination of
        incident, stress and harm model.
        ...
        \subsection{Failure model}
            ...
            historical data are using previous UAV crash, and its UAV specification as a starting point for data
            interpolation and extrapolation for calculating \ac{MTBF} \cite{breunig_modeling_2018} and \cite{weibel_safety_2012} for example.
            ...
            decompositions of UAS that may be used to create such a model are given \cite{burke_system-level_2011}.
                ...
                To assess the system's dependability, other methods can be utilized, such as \ac{FTA},
            \ac{ETA} , and \ac{FMECA} \cite{barr_preliminary_2017}. \ac{BBN} \cite{ancel_real-time_2017} have also been used
        ...
        \subsection{Impact location model}
            ...
            on the possible maximum UAV descent trajectory distance.
                ...
                The impact location model can be formulated using two different models, hypothetical \cite{ball_crash_2012},\cite{burke_system-level_2011},\cite{primatesta_ground_2020},\cite{primatesta_ground_2020}
            and empirical model.
            ...
            model such as in assumes the UAV will continue descending based on their glide profile. The planform model
            ...
            \cite{primatesta_ground_2020} added parachute descent, an event where UAV deployed its parachute when descending. And fly away
            ...
            turn area) of the impact distribution for fixed-wing UAV. However, like most models, it assumes a uniform
        ...
        \subsection{Recovery model}
            ...
            of recovery. For instance, if the UAV malfunctions and enters a lost-of-control failure state, the
        ...
        \subsection{Stress model}
            ...
            calculated was human physical harm modeled in \ac{KE}. While secondary effects such as explosions and the release of
            hazardous chemicals will have distinct related stress qualities that need to be modeled differently. There
            are model proposed by \cite{ball_crash_2012} to analyze the consequences of heat radiation and explosions which often produces
        ...
        \subsection{Incident stress model}
            ...
            presumed that the UAV KE is entirely directed directly at the human. Although a conservative assumption,
            ...
            In the model described by \cite{ball_crash_2012}, the amount of KE that is absorbed by the shelter as a result of its
